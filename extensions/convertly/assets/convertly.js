...
(function () {
  ...
  //
  // Always visible the instant it renders — it used to fade in only once
  // the buy-now form scrolled out of view (via IntersectionObserver, or a
  // scroll-position fallback), but that depended on correctly finding that
  // form in the first place, which varied unpredictably across themes and
  // was the whole reason it went missing entirely on some of them. Simpler
  // and more reliable: no detection to get wrong, no delay, always there.
  function renderSatc(s) {
    ...
    fetch('/products/' + handle + '.js')
      ...
      .then(function (product) {
        ...
        var bar = document.createElement('div');
        bar.className = 'cb-satc cb-satc--visible';
        bar.id = 'cb-satc';
